  ...
  public:
    ...
    void DFS(int i, int *v)
    {
        ...
        for (int j = 0; j < G->vexnum; j++)
            if (G->arcs[i][j] != 0 && G->arcs[i][j] != INT_MAX && !v[j])
                DFS(j, v);
    }
    ...
    {
        UF u(G->vexnum);
        sort(G->arc.begin(), G->arc.end(), [](Edge &a, Edge &b) -> int { return a.value < b.value; });
        int ret = 0, num = 1;
        for (auto &t : G->arc) {
            if (u.Union(t.s, t.e)) {
                ret += t.value;
...
int main()
{
    ...
    for (int j = 1; j <= 3; j++) {
        ...
        for (int k = 0; k <= 1; k++) {
            cout << t[k] << "算法平均执行时间：" << time[k] / 100 << "ms 最大执行时间: " << maxtime[k]
