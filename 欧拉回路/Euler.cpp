  ...
  public:
    ...
    bool RandCreate(int type, int vex, int arc)
    {
        ...
        while (G->arcnum < arc - 1) {
            t = s[pre].getRandom();
            if ((G->type == Undirected_graph || G->type == Undirected_net) && s[t].nums.size() == 1)
                continue;
            s[pre].remove(t);
            InsertEdge(pre, t);
            pre = t;
    ...
    bool DFS(int t, int pre, int anc)
    {
        ...
        for (int i = 0; i < s; i++) {
            ...
            if ((G->type == Undirected_graph || G->type == Undirected_net) && v == pre)
                continue;
            if (v == anc)
                return false;
            if (!visited[v] && !DFS(v, t, anc))
                return false;
        }
        return true;
    }
...
int main()
{
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 3; j++) {
            ...
            for (int k = 0; k <= 1; k++) {
                cout << al[k] << "算法平均执行时间：" << time[k] / 100 << "ms" << endl << endl;
                f[k].close();
            }
        }
    }
